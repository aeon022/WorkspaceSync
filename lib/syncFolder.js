...
export async function deleteDeviceFile(handle, deviceId) {
  try {
    await handle.removeEntry(`${deviceId}.json`);
    try {
      await handle.removeEntry(`${deviceId}-inbox.json`);
    } catch {
      // Ignore if inbox does not exist
    }
    return true;
  } catch (err) {
    console.warn('[DeckMirror] could not delete device file', deviceId, err);
    return false;
  }
}
