...
export async function scanSyncFolder(handle, ownDeviceId) {
  ...
  for await (const [name, entry] of handle.entries()) {
    ...
    if (name === LAYER2_FILE_NAME) continue; // this device's own local live-tab file, not a device snapshot
    ...
    if (placeholderMatch) {
      const placeholderName = placeholderMatch[1].endsWith('.json')
        ...
        : placeholderMatch[1];
      if (placeholderName.endsWith(INBOX_SUFFIX) || placeholderName === LAYER2_FILE_NAME) continue;
