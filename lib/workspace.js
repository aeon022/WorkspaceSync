...
// How often the Layer 2 UI mod (uimod/deckmirror-uimod.js) writes a
...
// uimod/deckmirror-uimod.js) - messaging between Vivaldi's privileged UI
...
const WORKSPACE_ID_DEBOUNCE_KEY = 'workspaceIdDebounce';

// Vivaldi's own vivExtData.workspaceId is confirmed (multiple Vivaldi forum
// reports) to be racy right after tab creation or a workspace switch: a tab
// can transiently report the currently-ACTIVE workspace instead of its real
// one until Vivaldi's internal bookkeeping settles. A single point-in-time
// read of it isn't trustworthy enough to sync/mirror on, so a workspaceId
// change is only accepted once it's been seen on two consecutive polls
// (~1min apart, matching the background alarm); until then the
// previously-confirmed value keeps being reported. Rebuilding the stored
// state from only the tab ids seen this pass also prunes closed tabs for
// free, so a reused id never inherits a stale debounce entry.
export async function debounceWorkspaceIds(rawByTabId) {
  const { [WORKSPACE_ID_DEBOUNCE_KEY]: stored } = await chrome.storage.local.get(WORKSPACE_ID_DEBOUNCE_KEY);
  const state = stored || {};
  const nextState = {};
  const resolved = new Map();

  for (const [tabId, candidate] of rawByTabId) {
    const entry = state[tabId];
    let value;
    if (!entry || candidate === entry.confirmed) {
      value = candidate;
      nextState[tabId] = { confirmed: candidate, pending: null };
    } else if (candidate === entry.pending) {
      value = candidate;
      nextState[tabId] = { confirmed: candidate, pending: null };
    } else {
      value = entry.confirmed;
      nextState[tabId] = { confirmed: entry.confirmed, pending: candidate };
    }
    resolved.set(tabId, value);
  }

  await chrome.storage.local.set({ [WORKSPACE_ID_DEBOUNCE_KEY]: nextState });
  return resolved;
}
...
export async function getLocalWorkspaces() {
  ...
  const rawByTabId = new Map(localTabs.map((tab) => {
  ...
  const debouncedByTabId = await debounceWorkspaceIds(rawByTabId);
  ...
  return groupTabs(localTabs.map((tab) => ({
    ...
    favIconUrl: tab.favIconUrl,
    index: tab.index,
    workspaceId: debouncedByTabId.get(tab.id)
  })));
}
